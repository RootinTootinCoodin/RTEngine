...
bool ModuleFileSystem::Init(JSON_Object* config)
{
	...
	char* write_path = SDL_GetPrefPath(App->organization.c_str(), App->window->name.c_str());
...
bool ModuleFileSystem::CopyFromOutsideFS(const char * full_path, const char * destination)
{
	...
	{
		LOG("File System error while copy from [%s] to [%s], error: %s", full_path, destination,PHYSFS_getLastError());
	}
